...
const log = createLogger("api/tokens/init");
const BACKEND_URL = process.env.BACKEND_URL ?? "http://localhost:8000";

export async function POST(req: NextRequest) {
  log.info("POST", "START | incoming token init request");
  ...
  log.info("POST", `forwarding to backend | device_id=${device_id.slice(0, 8)}...`);

  try {
    const res = await fetch(`${BACKEND_URL}/api/tokens/init`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ device_id: device_id.trim(), user_id }),
    });
    ...
    log.info("POST", `END | success | total_tokens=${data?.total_tokens}`);
