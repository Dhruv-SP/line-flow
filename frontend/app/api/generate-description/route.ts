...
const log = createLogger("api/generate-description");
const BACKEND_URL = process.env.BACKEND_URL ?? "http://localhost:8000";

export async function POST(req: NextRequest) {
  log.info("POST", "START | incoming generate description request");
  ...
  const { prompt, device_id, thread_id, user_id } = body as {
    prompt?: string;
    device_id?: string;
    thread_id?: string;
    user_id?: string;
  ...
  log.info("POST", `forwarding to backend | prompt_length=${prompt.trim().length}`);

  try {
    const res = await fetch(`${BACKEND_URL}/api/generate_description`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt: prompt.trim(), device_id, thread_id, user_id }),
    });
    ...
    log.info("POST", `END | success | description_length=${data?.description?.length ?? 0}`);
